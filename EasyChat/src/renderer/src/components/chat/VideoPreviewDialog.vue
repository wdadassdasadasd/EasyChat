    ...
    >
        <div class="video-preview-panel">
            ...
            <div class="video-footer">
                ...
                <el-progress
                    v-if="loading || downloadState?.status === 'downloading'"
                    class="video-footer-progress"
                    :percentage="footerProgress"
                    :indeterminate="footerProgress <= 0"
                    :show-text="false"
                    :stroke-width="4"
                />
                <span
                    v-if="downloadState?.status === 'done'"
                    class="video-download-path"
                    :title="downloadState.path"
                >{{ downloadState.path }}</span>
                <span v-if="downloadState?.status === 'failed'" class="video-download-error">
...
const safeDownloadProgress = computed(() => Math.min(100, Math.max(0, Number(props.downloadState?.progress) || 0)));
const footerProgress = computed(() => props.downloadState?.status === 'downloading' ? safeDownloadProgress.value : safeProgress.value);
const downloadButtonText = computed(() => {
    if (props.downloadState?.status === 'downloading') {
        return `下载中 ${safeDownloadProgress.value}%`;
    }
