...
### 3.4 P3 级别 — 优化建议 ✅ 已全部修复

#### BUG-P3-1: 临时视频文件无清理策略 ✅ 已修复

- **文件**: `src/main/ipc.js` L352
- **现象**: 临时视频文件写入 `app.getPath('temp')/EasyChat/video-preview/` 但无定时清理
- **修复方案**: 应用启动或退出时清理超期临时文件
- **实际修复**: 新增 `src/main/tempVideoFiles.js`，应用启动与退出时尽力删除超过 24 小时的预览文件；目录不存在或单文件删除失败不会阻塞应用生命周期。

#### BUG-P3-2: ffmpeg 视频封面提取无超时 ✅ 已修复

- **文件**: `src/main/ipc.js` L407-448
- **现象**: `spawn('ffmpeg', ...)` 无进程超时，ffmpeg 卡住时 Promise 永不 resolve
- **修复方案**: 为 spawn 增加超时 kill
- **实际修复**: 封面提取收口到 `src/main/uploadSourceRegistry.js`，增加 10 秒定时器；超时后终止 ffmpeg、清理定时器并返回失败，由既有浏览器截帧逻辑继续回退。

#### BUG-P3-3: 全局注册 Vue 组件过多 ✅ 已修复

- **文件**: `src/renderer/src/main.js`
- **现象**: 15+ 个 Element Plus 图标组件全局注册，增加 bundle 体积
- **修复方案**: 按需引入或使用 `unplugin-icons`
- **实际修复**: 移除 15 个全局图标注册，静态图标改为组件内导入，导航等动态图标配置保存 `markRaw` 组件对象；未新增插件或依赖，通用业务组件注册保持不变。
- **构建变化**: renderer 主入口由 2,467.17 kB 降至 2,466.40 kB，Chat chunk 由 265.97 kB 降至 265.67 kB。

#### BUG-P3-4: DB 初始化失败后 dbReadyPromise 只打印不熔断 ✅ 已修复
...
- **实际修复**: 保留启动熔断，公开读写入口统一经过 `ensureDbReady`；停止导出仅供初始化使用的 `createTable`，初始化失败后 `dbReady` 与后续查询一致拒绝且不会执行 SQL。

#### BUG-P3-5: 消息批量写入仍逐条 INSERT ✅ 已修复

- **文件**: `src/main/db/ADB.js`、`src/main/db/ChatMessageModel.js`
- **现象**: `saveMessageBatch` 虽在事务内执行，但消息主表仍逐条 INSERT，批量接收时 SQL 调度开销偏高
- **实际修复**: 新增 `insertOrReplaceManyStrict(tableName, rows)`，按有效列集合分组，并按 SQLite 500 参数上限分块生成多行 SQL；`saveMessageBatch` 已切换到批量主表写入，事务、去重、会话摘要、未读计数和 FTS 更新语义保持不变。

**P3 修复验证（2026-06-13）**: Vitest 全量测试 263 项通过、4 项跳过；生产构建通过；本次生产代码与可解析回归测试的只读 ESLint error-level 检查通过。`test/main/stress/chatMessageStress.spec.js` 使用 class fields，当前 ESLint 8 配置解析失败，但该文件已由 Vitest 正常执行并通过。
...
| 问题 | 位置 | 严重度 |
|------|------|--------|
| `isRequestFailure` 在 `useChatMessageSender.js` 和 `mediaUploadTransport.js` 中重复定义 | ✅ 已抽取到 `utils/RequestFailure.js` 并复用 | P3（已修复） |
| `getReceiveContactId` 在 `wsClient.js` 和 `useChatMessages.js` 中逻辑重复 | 实现略有差异 | P2 |
| `getSendFailureMessage` / `getUploadFailureMessage` 错误消息硬编码在 composable 中 | ✅ 已与请求失败分类一并抽取到共享工具 | P3（已修复） |
| IPC callback channel 命名不统一 | ✅ 已建立 `src/shared/ipcChannels.js` 常量并由 main/preload 共用，wire 字符串保持兼容 | P3（已修复） |
...
| DB 初始化 | `ensureDbReady` gate | ✅ 公开读写入口全覆盖，初始化失败一致拒绝 |
| 消息顺序 | 串行处理队列保证 | ✅ 已实现 |
...
| 重复消息 | DB 去重 + renderer `messageIdSet` 双重防护，批量重复场景已回归验证 | P3（已验证） |
...
| FTS5 | ✅ | 可选，失败降级 LIKE |
| DB 初始化失败 | ✅ | 启动熔断 + 公开读写入口 `ensureDbReady` 门禁，失败回归测试已覆盖 |
...
| ID | 问题 | 收益 | 风险 | 工作量 | 状态 |
|----|------|------|------|--------|------|
| P3-1 | 临时文件清理 | 磁盘空间 | 低 | 0.5天 | ✅ 已修复 |
| P3-2 | ffmpeg 超时 | 进程不卡死 | 低 | 0.5天 | ✅ 已修复 |
| P3-3 | 组件按需引入 | 包体积减小 | 低 | 1天 | ✅ 已修复 |
| P3-4 | DB ensureDbReady 全覆盖 | 初始化安全 | 低 | 1天 | ✅ 已修复 |
| P3-5 | 批量 INSERT 优化 | 写入性能 | 中 | 2天 | ✅ 已修复 |
...
**批准合并和发布。** 本轮复核覆盖最近 P1/P2/P3 提交及当前完整项目，重点检查了聊天收发、SQLite 写入与未读数、WebSocket 初始化与恢复、Electron IPC、preload 白名单、文件生命周期和会话交互。未发现仍需阻断发布的已知缺陷。

### 本轮修复

1. INIT 最近消息仅用于本地回填，不再叠加服务端会话列表携带的权威未读数。
2. 补齐已有 `delChatSessionCallback` 的 preload 订阅；删除会话持久化失败或超时时恢复 renderer 会话状态。
3. 外部窗口仅允许打开 HTTP(S) URL，并吸收 `shell.openExternal` 异常。
4. 本地视频读取与打开要求路径属于当前用户消息记录；主进程拒绝空文件、非文件及超过 128 MiB 的读取。
5. 下载文件的打开和定位要求真实路径位于当前下载目录内，阻止目录穿越与符号链接越界。
6. 恢复 4 个 IPC 正常路径测试，清除 5 个 ESLint error、遗留 FIXME、用户目录及高频消息载荷调试日志。

### 验证结果

| 检查项 | 结果 |
|--------|------|
| `npm test -- --run` | 26 个测试文件通过，281 项测试通过，0 失败，0 跳过 |
| 只读 ESLint | 0 error；保留 11,631 条既有 Prettier/CRLF warning |
| `npm run build` | 通过 |
| `git diff --check` | 通过 |
| 构建产物与依赖 | `out/`、`node_modules/` 保持忽略；依赖及锁文件未修改 |

### 既有警告与残余风险

- 构建仍提示 `wsClient.js` 同时被静态和动态导入；当前不影响产物正确性。
- Element Plus 样式链仍产生 Dart Sass legacy API 与 `if()` 弃用提示，属于第三方依赖升级事项。
- 本轮使用 mock 覆盖了协议兼容、错误回调、路径越权和回滚；未连接真实后端进行登录、断网重连、跨账号数据库和大文件下载的端到端联调，发布前仍应按目标环境执行一次人工冒烟。
- 未修改 HTTP/WebSocket 协议、IPC wire channel、既有 preload 方法、SQLite 表结构、索引、迁移或用户数据格式。
