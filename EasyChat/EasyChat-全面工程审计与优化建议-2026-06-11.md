...
#### BUG-P2-2: 请求去重 key 依赖 JSON.stringify
...
| S-6 | 无 IPC 输入校验 | P2 | 待修复 | renderer 可发送畸形数据（但白名单限制了通道范围） |
| S-7 | 非标准 auth header | P2 | 待修复 | `token` header 而非 `Authorization: Bearer` |
...
登录 → HTTP POST /account/login → response.data.token
    ...
    → electron-store token (明文)
    → HTTP header: { token: xxx }
    → WS URL: ws://domain?token=xxx
```

**问题**:
1. Token 在 renderer localStorage 中明文存储
2. Token 被复制到 electron-store
3. Token 通过自定义 header 发送
...
| ID | 问题 | 收益 | 风险 | 工作量 | 验证方式 |
|----|------|------|------|--------|---------|
| P2-1 | IPC 输入校验 | 安全增强 | 低 | 2天 | 模糊测试 |
| P2-2 | 请求去重 key | 去重准确 | 低 | 0.5天 | 不同 key 顺序测试 |
| P2-3 | macOS dock 行为 | macOS 体验 | 低 | 0.5天 | macOS 测试 |
| P2-4 | 消息长度校验 | 防止超长消息 | 低 | 0.5天 | 发送超长文本 |
| P2-5 | 发送队列上限 | 内存安全 | 低 | 0.5天 | 连续快速发送 |
| P2-6 | electron-store 加密 | 数据安全 | 低 | 0.5天 | 检查 store 文件 |
| P2-7 | 标准 auth header | 合规性 | 中 | 1天 | 后端联调 |
